...
class ResponseUtil:
    ...
    def success_new(
            ...
            rows: Optional[Any] = None,
            dict_content: Optional[Dict] = None,
            model_content: Optional[Union[BaseModel, Dict]] = None,
            headers: Optional[Mapping[str, str]] = None,
            media_type: Optional[str] = None,
            background: Optional[BackgroundTask] = None,
    ) -> Response:
        ...
        if model_content is not None:
            if isinstance(model_content, BaseModel):
                result.update(model_content.model_dump(by_alias=True))
            elif isinstance(model_content, dict):
                result.update(model_content)
            else:
                result["model_content"] = model_content  # 可选兜底字段
